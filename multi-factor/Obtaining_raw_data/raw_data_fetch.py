...
class RawDataFetcher(FactorGenerater):
    ...
    def ensure_data(self, func, save_dir, start_dt='20200101', end_dt='20201231'):
        """ 确保按交易日获取数据
        """
    ...
    @retry(stop_max_attempt_number=500, wait_random_min=1000, wait_random_max=2000)
    def ensure_data_by_q(self, func, save_dir, start_dt='20200101', end_dt='20201231'):
        """ 确保按季度获取数据
        """
        tmp_dir = os.path.join(self.root, save_dir)
        dl = [pd.to_datetime(name.split(".")[0]) for name in os.listdir(tmp_dir)]
    ...
    def create_indicator_m_by_d(self, raw_data_dir, raw_data_field, indicator_name, start_dt='20200101', end_dt='20201231'):
        ''' 通过日频数据创建月频指标
        '''
        tmp_dir = os.path.join(self.root, raw_data_dir)
        s = pd.to_datetime(start_dt)
    ...
    def create_indicator_m_by_d_ex(self, raw_data_dir, raw_data_field, indicator_name, start_dt='20200101', end_dt='20201231'):
        ''' 通过日频数据创建月频指标
        '''
    ...
    def create_indicator_m_by_q(self, raw_data_dir, raw_data_field, indicator_name, start_dt='20200101', end_dt='20201231'):
    ...
    def create_indicator_m_by_q_ex(self, raw_data_dir, raw_data_field, indicator_name, start_dt='20200101', end_dt='20201231'):
        ''' 通过季频数据创建月频指标,主要用于财报数据处理
        '''
        s = pd.to_datetime(start_dt) #统计周期开始
        e = pd.to_datetime(end_dt) #统计周期结束
...
#######################################################
    ...
    def create_mkt_cap_float_m(self):
        ...
        s = pd.to_datetime('20200101')
        e = pd.to_datetime('20201231')
        new_tdays = self._get_trade_days(s, e, "M")
        new_caldays = [self._get_month_end(tdate) for tdate in new_tdays]
        all_stocks_info = self.meta
        df = pd.DataFrame(index=all_stocks_info.index, columns=new_caldays)
