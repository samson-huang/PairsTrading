...
class Top5Strategy(bt.Strategy):
...
class MultiStocksStrategy(bt.Strategy):
    ...
    def __init__(self):
        ...
        self.datas = {}
        # 每只股票的持仓比例
        self.pos_ratios = [1 / 5] * 5
        # 占用资金总比例
        self.used_cash = 0

    def next(self):

        avbl_stocks = [data for data in self.datas.values()
                       if data.close[0] > 0]

        # 根据score列进行排序
        avbl_stocks.sort(key=lambda x: x.score[0], reverse=True)

        for data in self.datas:
            if data not in avbl_stocks[:5]:
                self.sell(data)

        for ratio, stock in zip(self.pos_ratios, avbl_stocks[:5]):
            cash_for_stock = self.broker.cash * (1 - self.used_cash) * ratio
            shares = int(cash_for_stock / stock.close[0])
            self.buy(stock, shares)
            self.used_cash += ratio