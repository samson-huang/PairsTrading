...
WORK_PATH = "C://funds_data//"
class WindQueryFailError(Exception):
    pass
class TuShareQueryFailError(Exception):
    pass
...
class Data:
    ...
    def close_file(self, df, name, **kwargs):
        if name == 'meta':
            ...
            self.__update_frepmap()
        #self.__update_attr(name)----更新第一列数据tushare接口暂时不需要
...
#        return series
    ...
    def __getattr__(self, name):
        if name not in self.__dict__:
            self.__dict__[name] = self.open_file(name)#meta就是从这里获取得
...
class FactorProcess:
    ...
    def __update_tradedays(self):
        ...
        endday = datetime.combine(datetime.now(), time.min)
    ...
    def open_file(self, path):
        if path.endswith('.csv'):
            return pd.read_csv(path, index_col=[1], engine='python')
        else:
