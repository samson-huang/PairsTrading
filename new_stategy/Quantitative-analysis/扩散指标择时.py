...
def TdaysOffset(end_date: str, count: int) -> int:
    ...
    '''
    df = my_pro.trade_cal(exchange='SSE',is_open=1)
    test_2=df[df['cal_date']=='20220421'].index[0]
    count_temp=test_2+count
    return df[df.index==count_temp]['cal_date'].values[0]
 '''   
...
def CreatMaSingal(index_symbol: str, start_date: str, end_date: str, N, N1,
    ...
    close_df = my_pro.pro.daily(ts_code=st, start_date=begin_date,
     end_date=end_date)
    #close_df = get_price(
