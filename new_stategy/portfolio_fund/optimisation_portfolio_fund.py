...
convertible_bond_code=(['163817.OF','000008.OF','000051.OF','006104.OF'])
...
# writer to csv
outdata.to_csv ("C:\demo_test\outdata.csv" , encoding = "utf-8")
###########################################################################
"""
...
while date_index_iter+shift  <= end_index:
 ...
 ################################# 
  ...
  date2 = index[date_index_iter+shift]  #取t+1日期
  temp1 = outdata.loc[date2]/outdata.loc[date]  #得到t+1日收益率
  ...
  total_value = np.sum(loop_returns[index[date_index_iter]]*temp2*temp1) #根据t日算出的持仓比例，算出t+1日的累积收益
  # Increment Datedate
...
#############
date_index_iter=original_returns.shape[0]-1
date = index[date_index_iter]
#index_returns=original_returns[date_index_iter-1:date_index_iter+1]
index_returns=original_returns[1:date_index_iter+1]
index_returns.insert(original_returns.shape[1],'InterestRate',np.zeros(date_index_iter))
